...
The generation skill makes an adaptive dispatch decision based on how much data the extraction produced. For smaller databases (under 50KB of extracted text), a single high-capability agent receives the full job specification and all 17 input files and produces all 8 output documents in one pass. For larger databases, the work fans out to parallel workers that each own a disjoint slice of the output, followed by parallel critic agents that review every document under a sliding window, and a final summarizer agent that reconciles their feedback before writing the result. The 17-file pipe-delimited format produced by extraction templates is the central contract: engine-specific SQL templates produce it, the generation job spec consumes it, and nothing else in the plugin needs to understand either the source database dialect or the output document format.
...
3. `/generate-docs` command reads 17 extraction files from `references/databases/{DB_NAME}/`, adaptively chooses single-pass or fan-out based on corpus size, and produces 8 reference doc targets in `docs/database_reference/{DB_NAME}/`.
...
7. The 17-file pipe-delimited interface is the contract between extraction and generation. Engine-specific templates produce it; the generation job spec consumes it.
...
- **corpus**: The combined content of the 17 pipe-delimited extraction files for a given database. Used as a measure of total input size to decide between single-pass and fan-out generation.
- **17-file interface**: The contract between extraction and generation: exactly 17 pipe-delimited text files, one per schema information category, produced by an engine-specific SQL template and consumed by the engine-agnostic generation job spec.
- **job spec (job-spec.md)**: A generation specification written as a model prompt. Defines the expected inputs (the 17 files), the 8 output documents to produce, processing rules, and style requirements. Engine-agnostic by design.
...
schema-doc-generator/
  ...
  skills/
    generate-extraction-script/
      ...
      templates/
        mssql.sql                       # 17-section T-SQL extraction (from .github/scripts/)
...
[generate-extraction-script]
  ...
  |-- Creates: references/databases/{DB_NAME}/ with 17 placeholders
  |-- Presents script to user
  |
  === HUMAN GATE: user executes script ===
  |
  v
17 pipe-delimited text files in references/databases/{DB_NAME}/
  |
  v
[generate-reference-docs]
  |-- Validates 17 files exist (empty is OK for some sections)
...
**The 17-file interface** is the central contract. Extraction templates produce these files; the generation job spec consumes them. Each file is pipe-delimited with a header row. The mapping:
...
| `17_lookup_data.txt` | Actual data from lookup tables (<100 rows) for enriching documentation |

Files 07, 12, 13, 14, 15, 17 are commonly empty depending on database design. The generation skill must handle empty files gracefully.
...
The skill measures total byte size of the 17 input files:

- **<50KB:** Single `ed3d-basic-agents:opus-general-purpose` agent receives the full job spec and all 17 files. Produces all 8 output targets in one pass. No critic review.
- **>=50KB:** Fan-out with 2 workers (opus, parallel), 6 critics (sonnet, parallel after workers), 1 summarizer (opus, sequential after critics). Workers read all 17 files but own disjoint output segments. Critics verify accuracy against source data.
...
- `.github/scripts/schema_extraction.sql` — 431-line T-SQL extraction script with 17 sections, compatible with SQL Server 2008+ (compat level 100). Uses `FOR XML PATH` instead of `STRING_AGG`.
...
- `skills/generate-extraction-script/templates/mssql.sql` — the 17-section T-SQL extraction script (sourced from `.github/scripts/schema_extraction.sql`)
...
**Goal:** The `generate-reference-docs` skill can generate all 8 reference documents from a set of 17 extraction files using a single opus agent.
...
**Done when:** `/generate-docs` invocation reads 17 extraction files, dispatches an opus agent with the job spec, and writes 8 reference documents to the correct output directory. Handles empty input files gracefully.
<!-- END_PHASE_3 -->
...
**Multi-engine extensibility:** Adding MySQL or PostgreSQL requires only a new `templates/{engine}.sql` file and a command-pattern block in the extraction skill's SKILL.md. The 17-file interface and generation job spec are engine-agnostic. The extraction SQL templates will need to produce equivalent output despite differences in system catalog schemas (`information_schema` vs `pg_catalog` vs `sys.*`).
